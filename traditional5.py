...
if True:
	...
	def create_BK(index_gen, make_atom_memory, make_rep_memory, node_to_bs, bs_to_det, make_atom_prot, \
					make_rep_prot, make_det_prot, make_bs_prot, classical_delay = 0.):
		# Inputs:
		...
		#	 classical_delay = time delay (in ns) for classical connection between nodes and bs
		# Returns:
...
def run_simulation(num_channels, atom_times, rep_times, channel_loss, duration = 100,\
	...
	REPEATER_CONTROL_RATE = link_time 	# How often the repeater control sends start signals 
										# to repeater nodes that are eligible. 
										# (For RepeaterControlProtocol, where run_protocol is run.)
	...
	make_BK = lambda index_generator = None: \
				create_BK(index_gen if index_generator is None else index_generator, make_atom_memory, \
										...
										make_rep_prot, make_det_prot, make_bs_prot, link_delay)
