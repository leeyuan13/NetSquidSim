...
from hybrid9 import run_simulation as run_hybrid
from traditional2 import run_simulation as run_trad
...
#atom_times = [[1, 1e-3],]
#rep_times = [[1, 1e-3], [1, 1e-3]]
#atom_times = [[1e9, 1e-3],]
#rep_times = [[1e9, 1e-3], [1e9, 1e-3]]
...
duration = 1000
#duration = 100
repeater_channel_loss = 1e-5
noise_on_nuclear_params = [0.05, 0.05]
...
for m in num_qubits:
	...
	for k in range(num_repeats):
		print('hybrid', m, k)
		result = run_hybrid(int(m/2), atom_times, rep_times, channel_loss, duration, repeater_channel_loss, noise_on_nuclear_params).data
		...
		print('trad', m, k)
		result = run_trad(m, atom_times, rep_times, channel_loss, duration, None, noise_on_nuclear_params).data
...
#	data_trad.append([])
#	for k in range(num_repeats):
#		with open(PREFIX+'run_test2_data_hybrid_'+str(m)+'_trial_'+str(k)+'.pickle', 'rb') as fn: 
#			result = pickle.load(fn)
#		data_hybrid[-1].append(result)
#		with open(PREFIX+'run_test2_data_trad_'+str(m)+'_trial_'+str(k)+'.pickle', 'rb') as fn: 
#			result = pickle.load(fn)
#		data_trad[-1].append(result)
