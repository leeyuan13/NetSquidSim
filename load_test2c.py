...
def load_test2_data_all(DIRECTORY, num_qubits, num_repeats):
	...
	return (avg_rates, avg_wait_times, avg_electron_attempts, avg_fidelities), \
			(rates, wait_times, electron_attempts, fidelities), (data_hybrid, data_trad)

def load_test2_data(DIRECTORY, num_qubits, num_repeats):
	return load_test2_data_all(DIRECTORY, num_qubits, num_repeats)[:2]
...
if __name__ == '__main__':
	...
	params = {'T2_dom1/': {'T1': 1e12, 'T2': 1e-3, 'num_qubits': num_qubits[:], 'channel_loss': 0.10,\
			  ...
			  'T2_dom3/': {'T1': 1e12, 'T2': 1e3, 'num_qubits': num_qubits[:], 'channel_loss': 0.10,\
			  				'num_repeats': 5, 'duration': 1000},
			  'T2_dom4/': {'T1': 1e12, 'T2': 1e2, 'num_qubits': num_qubits[:], 'channel_loss': 0.10,\
			  				'num_repeats': 5, 'duration': 1000},
			  'T2_dom5/': {'T1': 1e12, 'T2': 1e1, 'num_qubits': num_qubits[:], 'channel_loss': 0.10,\
			  				'num_repeats': 5, 'duration': 1000}}
	
	if True:
		data = dict()
		directory_list = ['T2_dom'+str(i)+'/' for i in [1,2,5,3]]
	...
	if True:
		directory_list = ['T2_dom'+str(i)+'/' for i in [1,2,5,3]]
		color_list = [None]*4
		label_list = [r'$T_2 = 10^{-3}$', r'$T_2 = 10^{0}$', r'$T_2 = 10^{1}$', \
						r'$T_2 = 10^{3}$']
