...
class RepeaterControlProtocol(TimedProtocol):
	...
	def __init__(self, time_step, node, rep1, conn1, rep2, conn2, planner_conn = None, to_print = False):
		...
		self.repeater_bs_prot = None

		self.to_print = to_print

	def set_repeater_bs_prot(self, repeater_bs_prot):
		self.repeater_bs_prot = repeater_bs_prot
	...
	def is_entangled(self, bank, index, args = None):
			...
			if isinstance(data, tuple) and data[0] == "success":
				...
				# Success of local Barrett-Kok.
					...
					if self.status2[other_index][0] == 2:
						...
						self.repeater_bs_prot.change_switch('remove', index, other_index)
			...
			if isinstance(data, tuple) and data[0] == "success":
				...
				if data[1] == 1:
					...
					if self.status1[other_index][0] == 2:
						...
						self.repeater_bs_prot.change_switch('remove', other_index, index)
		...
		while len(self.queue1) * len(self.queue2) > 0:
			...
			# appropriately.
			self.repeater_bs_prot.change_switch('add', counter1, counter2)
...
class RepeaterBSProtocol(TimedProtocol):
	...
	The repeater control directly changes the switches in the repeater beam splitter.
	'''
...
class HybridRepeater:
	...
	def set_params(self, BS_CLOCK_RATE, REPEATER_CONTROL_RATE, repeater_channel_loss, local_delay,\
		...
		self.make_rep_bs_prot = lambda x, a, b, c, d: \
											RepeaterBSProtocol(self.BASE_CLOCK_RATE, x, a, b, c, d)
	...
	def make_repBS(self):
		...
		# Set up repeater_bs protocol.
		rep_bs_prot = make_rep_bs_prot(repeater_bs, conn1, conn2, test_conn1, test_conn2)
		# Inform repeater control about this connection.
		rep_control_prot.set_repeater_bs_prot(rep_bs_prot)
